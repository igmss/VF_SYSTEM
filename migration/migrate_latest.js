...
const EXPORT_FILE = path.join(__dirname, '../vodatracking-default-rtdb-export - 2026-04-28T233009.955.json');
...
async function clearAndRemigrate() {
  ...
  await supabase.from('daily_flow_summary').delete().neq('date_key', 'NON_EXISTENT');
  console.log(`  Cleared daily_flow_summary ✓`);
  
  await supabase.from('system_profit_snapshots').delete().neq('date_key', 'NON_EXISTENT');
  console.log(`  Cleared system_profit_snapshots ✓`);

  await supabase.from('system_config').delete().neq('key', 'NON_EXISTENT');
  console.log(`  Cleared system_config ✓`);
...
async function run() {
  console.log('===========================================');
  console.log(' Fresh Migration from: 2026-04-28 (Latest)');
  console.log('===========================================\n');
  ...
  await migrateTable('mobile_numbers', data.mobile_numbers, (id, item) => {
    ...
    delete row.name; // This column doesn't exist in Supabase schema
    return row;
  });
