...
console.log("Pay-Investor-Profit function started");
...
serve(async (req) => {
  ...
  try {
    ...
    if (role !== 'ADMIN') {
      return new Response(
        JSON.stringify({ error: "Only admins can pay investor profit" }),
    ...
    // 3. Call RPC
    const { data, error } = await supabase.rpc('record_investor_payout', {
      p_investor_id: investorId,
      p_amount: amount,
      p_bank_account_id: bankAccountId,
      p_notes: notes || "Investor Profit Payout",
      p_created_by_uid: uid
    });

    if (error) {
      console.error("[Pay Investor Profit] RPC Error:", error);
      return new Response(
    ...
    console.error("[Pay Investor Profit] Error:", error);
